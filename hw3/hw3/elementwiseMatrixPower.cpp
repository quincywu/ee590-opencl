...
//Kernel Name: elementwiseMatrixPower
//Configuration Name: progressiveArraySum
...
{ 
    ...
    string newCLFileName = "C:\\Users\\quincywu\\Documents\\GitHub\\ee590-opencl\\hw3\\hw3\\program.cl";
    ...
    kernel = clCreateKernel(program, "elementwiseMatrixPower", &err);
    ...
    printf("Creating buffer inputA...\n");


     //Creating buffer: inputA

    size_t bufferSize_inputA = 262144 * sizeof(cl_float);
    char * bufferData_0;
    bufferSize_inputA = ReadBinaryFile("C:\\Users\\quincywu\\Documents\\GitHub\\ee590-opencl\\hw3\\hw3\\bin_inputA.bin", &bufferData_0);
    cl_mem buffer_inputA;
     buffer_inputA = clCreateBuffer(context, CL_MEM_USE_HOST_PTR, bufferSize_inputA, bufferData_0, &err); 
    if (err != CL_SUCCESS)
    {
        printf("Error: Failed to create buffer buffer_inputA\n");
        return EXIT_FAILURE;
    }



     //Creating uniform variable: Kpower

    cl_int uniformKpower = {3};
    printf("Creating buffer outputB...\n");


     //Creating buffer: outputB

    size_t bufferSize_outputB = 262144 * sizeof(cl_float);
    cl_mem buffer_outputB;
    char * bufferData_2 = (char*)_aligned_malloc(bufferSize_outputB, 4096);
     buffer_outputB = clCreateBuffer(context, CL_MEM_USE_HOST_PTR, bufferSize_outputB, bufferData_2, &err); 
    if (err != CL_SUCCESS)
    {
        printf("Error: Failed to create buffer buffer_outputB\n");
        return EXIT_FAILURE;
    }
    ...
    err  = clSetKernelArg(kernel, 0, sizeof(cl_mem), &buffer_inputA);
    ...
    err  = clSetKernelArg(kernel, 1, sizeof(cl_int), &uniformKpower);
    ...
    err  = clSetKernelArg(kernel, 2, sizeof(cl_mem), &buffer_outputB);
    ...
    printf("Read(Map/Unmap) buffer buffer_inputA\n");
    cl_map_flags MapFlags(CL_MAP_READ);
    void *buffer_inputA_out = clEnqueueMapBuffer(commands, buffer_inputA, true, MapFlags, 0, bufferSize_inputA, 0, NULL, NULL, &err);
    if (err != CL_SUCCESS)
    {
        printf("Error: Failed to map buffer buffer_inputA\n");
        return EXIT_FAILURE;
    }

    std::ofstream outFilebuffer_inputA_out("buffer_inputA_out.bin", std::ios::out | std::ios::binary);
    if (!outFilebuffer_inputA_out.is_open())
    {
        printf("Could not open output file for write");
    }
    else
    {
        outFilebuffer_inputA_out.write((char*) buffer_inputA_out, bufferSize_inputA);
        outFilebuffer_inputA_out.close();
    }
    printf("buffer_inputA buffer was dumped to buffer_inputA_out.bin\n");
    ...
    err = clEnqueueUnmapMemObject(commands, buffer_inputA, buffer_inputA_out, 0, NULL, NULL);

    if (err != CL_SUCCESS)
    {
        printf("Error: Failed to unmap buffer buffer_inputA\n");
        return EXIT_FAILURE;
    }

    clReleaseMemObject(buffer_inputA);

    printf("Read(Map/Unmap) buffer buffer_outputB\n");
    void *buffer_outputB_out = clEnqueueMapBuffer(commands, buffer_outputB, true, MapFlags, 0, bufferSize_outputB, 0, NULL, NULL, &err);
    if (err != CL_SUCCESS)
    {
        printf("Error: Failed to map buffer buffer_outputB\n");
        return EXIT_FAILURE;
    }

    std::ofstream outFilebuffer_outputB_out("buffer_outputB_out.bin", std::ios::out | std::ios::binary);
    if (!outFilebuffer_outputB_out.is_open())
    {
        printf("Could not open output file for write");
    }
    else
    {
        outFilebuffer_outputB_out.write((char*) buffer_outputB_out, bufferSize_outputB);
        outFilebuffer_outputB_out.close();
    }
    printf("buffer_outputB buffer was dumped to buffer_outputB_out.bin\n");
    err = clEnqueueUnmapMemObject(commands, buffer_outputB, buffer_outputB_out, 0, NULL, NULL);

    if (err != CL_SUCCESS)
    {
        printf("Error: Failed to unmap buffer buffer_outputB\n");
        return EXIT_FAILURE;
    }

    clReleaseMemObject(buffer_outputB);

    printf("Output validation: ");
    if(verifyOutput("buffer_outputB_out.bin", "C:\\Users\\quincywu\\Documents\\GitHub\\ee590-opencl\\hw3\\hw3_part1\\bin_ref_outputB0.bin"))
    {
        printf("true\n");
    }
    else
	...
	cl_float* result = (cl_float*)_aligned_malloc(262144 * sizeof(cl_float), 4096);

	QueryPerformanceCounter(&performanceCountNDRangeStart);
	for (unsigned int i = 0; i < 262144; ++i) //arraysize (512,512,0)
	{
		result[i] = pow(inputA[i], uniformKpower);
	}
