	...
	for (; counter < 512 - 1; ++counter) { // then local[0] would be less than global size
		...
		if (global[0] % local[0] == 0) {
			for (unsigned int i = 0; i < iterations; ++i) {
				...
				if (kernel2_1) err = clEnqueueNDRangeKernel(commands, kernel2_1, dim, NULL, global, local, 0, NULL, &prof_event);
				if (kernel2_2) err = clEnqueueNDRangeKernel(commands, kernel2_2, dim, NULL, global, local, 0, NULL, &prof_event);
				if (kernel2_3) err = clEnqueueNDRangeKernel(commands, kernel2_3, dim, NULL, global, local, 0, NULL, &prof_event);
				if (CL_SUCCESS != err)
				{
					printf("Error: Failed to execute kernel!\n");
					//clReleaseKernel(kernel);
					if (kernel2_1) clReleaseKernel(kernel2_1);
					if (kernel2_2) clReleaseKernel(kernel2_2);
					if (kernel2_3) clReleaseKernel(kernel2_3);
					clReleaseProgram(program);
					clReleaseCommandQueue(commands);
					clReleaseContext(context);
					return EXIT_FAILURE;
