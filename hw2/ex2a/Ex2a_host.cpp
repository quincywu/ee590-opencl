...
#define NUM_STRUCTS 1
	...
	/*
	if (kernel1_2) {
		err = clSetKernelArg(kernel1_2, 0, sizeof(float), &fl);
		...
		err = clSetKernelArg(kernel1_2, 1, sizeof(float), &fl2);
	...
	*/
	...
	err = clEnqueueNDRangeKernel(commands, kernel1_1, dim, NULL, global, local, 0, NULL, NULL);
	//err = clEnqueueNDRangeKernel(commands, kernel1_2, dim, NULL, global, local, 0, NULL, NULL);
	if (CL_SUCCESS != err)
	{
