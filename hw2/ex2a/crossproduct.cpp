...
int main(int argc, char** argv)
{
	...
	generateInput2(inputB, vector_size, 1);
	...
	//kernel3_1 = clCreateKernel(program, "hw2_2_1kernel", &err);
	//kernel3_2 = clCreateKernel(program, "hw2_2_2kernel", &err);
	...
	if (kernel3_1) {
		...
		err = clSetKernelArg(kernel3_1, 3, sizeof(cl_mem), (void *)&buffer_outputD);

		if ((CL_SUCCESS != err))
		{
			LogError("Error: Failed to set kernel3_1 arg3 '%s'.\n", TranslateOpenCLError(err));
			return err;
		}
	}
	...
	if (kernel3_2) {
		...
		err = clSetKernelArg(kernel3_2, 3, sizeof(cl_mem), (void *)&buffer_outputD);

		if ((CL_SUCCESS != err))
		{
			LogError("Error: Failed to set kernel3_2 arg3 '%s'.\n", TranslateOpenCLError(err));
			return err;
		}
	}
